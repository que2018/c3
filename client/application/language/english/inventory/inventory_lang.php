<?php  if ( ! defined('BASEPATH')) exit('No direct script access allowed');

// Text
$lang['text_inventory'] 	  	           = 'Inventory';
$lang['text_warehouse'] 	  	           = 'Warehouse';
$lang['text_product'] 	  	               = 'product';
$lang['text_location'] 	  	         	   = 'location';
$lang['text_quantity'] 	  	               = 'quantity';
$lang['text_inventory_view'] 	  	       = 'View Inventory';
$lang['text_import_inventory'] 	  	       = 'Import Inventory';
$lang['text_confirm_delete'] 	  	       = 'Are you sure to delete this inventory?';
$lang['text_inventory_list_description']   = 'Display All Inventories';
$lang['text_import_inventory_description'] = 'Import inventory from excel file';
$lang['text_loading_locations']            = 'loading locations ..';
$lang['text_only_excel_will_accepted'] 	   = '( only Excel file will be accepted )';
$lang['text_drop_file_and_upload'] 	       = 'Drop files here or click to upload';
$lang['text_confirm_delete'] 	           = 'Are you sure to delete this inventory?';
$lang['text_inventory_add_success'] 	   = '<i class="fa fa-check-circle-o"></i>&nbsp;Inventory add success';
$lang['text_inventory_edit_success'] 	   = '<i class="fa fa-check-circle-o"></i>&nbsp;Inventory edit success';
$lang['text_inventory_delete_success'] 	   = '<i class="fa fa-check-circle-o"></i>&nbsp;Inventory delete success';
$lang['text_rows_imported'] 	           = '<strong>Total: %s; Success: %s; Fail: %s</strong>';

// Column
$lang['column_product'] 	  	           = 'Name';
$lang['column_location'] 	  	           = 'Location';
$lang['column_warehouse'] 	  	           = 'Warehouse';
$lang['column_quantity'] 	  	           = 'Quantity';
$lang['column_date_added'] 	  	           = 'Date Added';
$lang['column_date_modified'] 	  	       = 'Date Modified';

// Entry
$lang['entry_client'] 	  	               = 'Client';
$lang['entry_product'] 	  	               = 'Product';
$lang['entry_quantity'] 	  	           = 'Quantity';
$lang['entry_location'] 	  	           = 'Location';
$lang['entry_warehouse'] 	  	           = 'Warehouse';

// Error
$lang['error_row_sku_empty'] 	  	       = 'row%s: sku is empty';
$lang['error_row_quantity_empty'] 	  	   = 'row%s: quantity is empty';
$lang['error_row_sku_not_found'] 	  	   = 'row%s: sku is not found';
$lang['error_update_quantity_error'] 	   = 'Update quantity fail';

