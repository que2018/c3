<?php  if ( ! defined('BASEPATH')) exit('No direct script access allowed');

// Text
$lang['text_sku'] 	  	                 		= 'sku';
$lang['text_quantity'] 	  	                    = 'quantity';
$lang['text_name'] 	  	                 		= 'name';
$lang['text_street'] 	  	             		= 'street';
$lang['text_city'] 	  	                 		= 'city';
$lang['text_state'] 	  	             		= 'state';
$lang['text_country'] 	  	             		= 'country';
$lang['text_zipcode'] 	  	             		= 'zipcode';
$lang['text_store'] 	  	             		= 'store';
$lang['text_length'] 	  	             	  	= 'length';
$lang['text_width'] 	  	             	  	= 'width';
$lang['text_weight'] 	  	             	  	= 'weight';
$lang['text_height'] 	  	             	  	= 'height';
$lang['text_store_sale_id'] 	  	            = 'store order ID';
$lang['text_shipping_provider'] 	  	 		= 'shipping provider';
$lang['text_shipping_method'] 	  	     		= 'shipping method';
$lang['text_sale_products'] 	  	     		= 'sale products';
$lang['text_add'] 	  	             			= 'add';
$lang['text_save'] 	  	             			= 'save';
$lang['text_return'] 	  	             		= 'return';
$lang['text_total'] 	  	             		= 'total';
$lang['text_status'] 	  	             		= 'status';
$lang['text_order'] 	  	             		= 'Order';
$lang['text_unsolved_order'] 	  	     		= 'Unsolved Order';
$lang['text_order_add'] 	  	         		= 'Add Order';
$lang['text_order_edit'] 	  	         		= 'Edit Order';
$lang['text_order_view'] 	  	         		= 'View Order';
$lang['text_order_edit_id'] 	  	            = 'Edit Order(#%s)';
$lang['text_order_view_id'] 	  	            = 'View Order(#%s)';
$lang['text_order_list'] 	  	         		= 'Order List';
$lang['text_import_order'] 	  	         		= 'Import Order';
$lang['text_loading'] 	  	        		 	= 'loading ...';
$lang['text_pending'] 	  	     		        = 'Pending';
$lang['text_completed'] 	  	     		    = 'Completed';
$lang['text_return'] 	  	     		        = 'return';
$lang['text_download_sample'] 	  	     		= 'download sample';
$lang['text_checkout_order_note'] 	  	        = 'checkout for order #%s';
$lang['text_import_order_success'] 	  	 		= '<i class="fa fa-check-circle-o"></i>&nbsp;orders import success';
$lang['text_order_edit_success'] 	  	 		= '<i class="fa fa-check-circle-o"></i>&nbsp;order edit success';
$lang['text_order_list_description'] 	 		= 'Display all unsolved orders';
$lang['text_unsolved_order_list_description']  	= 'Display all orders';
$lang['text_only_excel_will_accepted'] 	 		= '( only Excel file will be accepted )';
$lang['text_import_order_description'] 	 		= 'Import order from Excel file';
$lang['text_drop_file_and_upload'] 	     		= 'Drop files here or click to upload';
$lang['text_code_hint'] 	  	    	 		= 'UPC / SKU / ASIN / Proudct Name';
$lang['text_rows_imported'] 	  	     		= '<strong>%s rows are imported</strong>';
$lang['text_confirm_delete'] 	  	     		= 'Are you sure to delete this order?';
$lang['text_success_rows_imported'] 	  	    = '<i class="fa fa-check-circle-o"></i>&nbsp;<strong>%s rows are imported</strong>';
$lang['text_error_rows_imported'] 	  	    	= '<i class="fa fa-exclamation-triangle"></i>&nbsp;<strong>%s rows are imported</strong>';

// Tab
$lang['tab_customer'] 	  	             		= 'Customer';
$lang['tab_product'] 	  	             		= 'Product';
$lang['tab_fee'] 	  	                 		= 'Fee';
$lang['tab_store'] 	  	                 		= 'Store';

// Entry
$lang['entry_select'] 	  	         		    = 'Select';
$lang['entry_total'] 	  	         		    = 'Total';
$lang['entry_status'] 	  	         		    = 'Status';
$lang['entry_tracking'] 	  	         		= 'Tracking';
$lang['entry_note'] 	  	             		= 'Note';
$lang['entry_name'] 	  	             		= 'Name';
$lang['entry_street'] 	  	             		= 'Street';
$lang['entry_street2'] 	  	             		= 'Street2';
$lang['entry_city'] 	  	             		= 'City';
$lang['entry_state'] 	  	             		= 'State';
$lang['entry_country'] 	  	             		= 'Country';
$lang['entry_zipcode'] 	  	             		= 'Zipcode';
$lang['entry_email'] 	  	             		= 'Email';
$lang['entry_phone'] 	  	             		= 'Phone';
$lang['entry_label'] 	  	             		= 'Label';
$lang['entry_length'] 	  	             	  	= 'length';
$lang['entry_width'] 	  	             	  	= 'Width';
$lang['entry_height'] 	  	             	  	= 'Height';
$lang['entry_weight'] 	  	             	  	= 'Weight';
$lang['entry_length_class'] 	  	     	 	= 'Length Class';
$lang['entry_weight_class'] 	  	     	  	= 'Weight Class';
$lang['entry_shipping_provider'] 	  	 		= 'Shipping Provider';
$lang['entry_shipping_service'] 	  	        = 'Shipping Service';
$lang['entry_store'] 	  	                    = 'Store';
$lang['entry_store_order_id'] 	  	            = 'Store Order ID';
$lang['text_sale_add_success']		            = '<i class="fa fa-check-circle-o"></i>&nbsp;order add success';
$lang['text_sale_edit_success']		            = '<i class="fa fa-check-circle-o"></i>&nbsp;order edit success';

// Column
$lang['column_order_id'] 	  	     		    = 'Order Id';
$lang['column_store_order_id'] 	  	     		= 'Store Order Id';
$lang['column_tracking'] 	  	         		= 'Tracking Number';
$lang['column_name'] 	  	            	 	= 'Name';
$lang['column_customer'] 	  	                = 'Customer';
$lang['column_amount'] 	  	            	 	= 'Amount';
$lang['column_product_name'] 	  	     		= 'Product Name';
$lang['column_upc'] 	  	     		 		= 'UPC';
$lang['column_sku'] 	  	     		 		= 'SKU';
$lang['column_quantity'] 	  	     	 		= 'Quantity';
$lang['column_qty'] 	  	     	 			= 'Qty';
$lang['column_length_short'] 	  	            = 'L';
$lang['column_width_short'] 	  	            = 'W';
$lang['column_height_short'] 	  	            = 'H';
$lang['column_weight_short'] 	  	            = 'W';
$lang['column_checkout_location'] 	  	     	= 'Checkout Location';
$lang['column_date_added'] 	  	         		= 'Date Added';
$lang['column_action'] 	  	         		    = 'Action';
$lang['column_date'] 	  	         		    = 'Date';

// Button
$lang['button_add_product'] 	  	     		= 'Add Product';
$lang['button_select_file'] 	  	     		= 'Select file';
$lang['button_print'] 	  	             		= 'Print';

// Error
$lang['error_file_type_not_excel'] 	  	 		= 'Error: file type is not excel';
$lang['error_sku_not_found'] 	  	     		= 'Error: the %sth row has sku <strong>%s</strong> that is not existed';
$lang['error_order_exist'] 	  	         		= 'Error: store order id <strong>%s</strong> is existed';
$lang['error_client_not_select'] 	  	 		= 'Error: client is not selected';
$lang['error_row_data'] 	  	         		= 'Error: the <strong>%sth</strong> row has invalid data';
$lang['error_client_empty'] 	  	     		= 'Error: client is empty';
$lang['error_code_empty'] 	  	         		= 'Error: code is empty';
$lang['error_client_product_not_found']  		= 'Error: the product of this client is not found';
$lang['error_shipping_provider_not_set'] 		= 'Error: shipping provider is not set';
$lang['error_shipping_method_not_set']   		= 'Error: shipping method is not set';
$lang['error_inventory_not_enough']      		= 'Error: inventory is not enough';
$lang['error_location_not_select'] 	  	 		= '<i class="fa fa-exclamation-triangle"></i>&nbsp;location is not selected';
$lang['error_store_not_select'] 	  	 		= '<i class="fa fa-exclamation-triangle"></i>&nbsp;store is not selected';



