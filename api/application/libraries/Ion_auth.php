...
class Ion_auth
{
	...
	{
		return $this->ion_auth_model->user($id)->row_array();
	}
	
	public function get_user_by_name($username)
	{
		$this->ion_auth_model->where('username', $username);		
		return $this->ion_auth_model->users()->row_array();
	}
	
	public function users($name = NULL, $limit = 0)
	{
		if ( $name !== NULL ) {
			$this->ion_auth_model->like('username', $name);
			$this->ion_auth_model->like('first_name', $name);
			$this->ion_auth_model->like('last_name', $name);
			$this->ion_auth_model->like('email', $name);
		}
		
		if ( $limit > 0 )
			$this->ion_auth_model->limit($limit);
	...
	{
		return $this->ion_auth_model->get_users_groups()->row_array();
	}
	
	public function errors()
	{
		return $this->ion_auth_model->errors_array(FALSE);
	}
	
	public function error()
	{
		$errors = $this->ion_auth_model->errors_array(FALSE);
