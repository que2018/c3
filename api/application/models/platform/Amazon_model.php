	...
	public function upload($store_id)
	{
		...
		{			
			...
			try {
				...
				file_put_contents("log.txt", serialize($response));
				
				try {
					$feed_id =  $response['FeedSubmissionId'];
					
					file_put_contents("lx.txt", $feed_id);
					
					$amz = new AmazonFeedResult('jiusite', $feed_id); 
					$amz->setFeedId($feed_id); 
					$amz->fetchFeedResult();
					$debug = $amz->getRawFeed();
					
					file_put_contents("lm.txt", serialize($debug));
			...
			{				
				$messages[] = array(
					'level'    => 0,
					'content'  => sprintf($this->lang->line('text_sync_tracking_note'), $count, $success, $fail)
				);	
			}
			else
			{				
