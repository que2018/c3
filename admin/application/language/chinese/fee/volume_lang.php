<?php  if ( ! defined('BASEPATH')) exit('No direct script access allowed');

// Text
$lang['text_title'] 	  	             	= 'Volume';
$lang['text_volume'] 	  	             	= 'Volume';
$lang['text_fee'] 	  	             	    = '费用';
$lang['text_save'] 	  	             	    = '保存 volume';
$lang['text_volume_level'] 	  	            = 'volume level';
$lang['text_volume_level_end'] 	  	        = 'volume level end';
$lang['text_volume_status'] 	  	        = '状态';
$lang['text_volume_sort_order'] 	  	    = '排序 订单';
$lang['text_edit_success'] 	  	            = '<i class="fa fa-check-circle-o"></i>&nbsp;编辑 费 成功';

// Entry
$lang['entry_status'] 	  	             	= '状态';
$lang['entry_sort_order'] 	  	            = '排序 订单';

// Column
$lang['column_volume'] 	  	             	= 'Volume';
$lang['column_amount'] 	  	            	= '总量 / inch^3';

// Tab
$lang['tab_level'] 	  	             		= 'Level';

// 错误 
$lang['error_volume_row_required']          = '<i class="fa fa-exclamation-triangle"></i>&nbsp;volume 在 行 <strong>%s</strong> 是 必须的';
$lang['error_amount_row_required']          = '<i class="fa fa-exclamation-triangle"></i>&nbsp;总量 在 行 <strong>%s</strong> 是 必须的';
